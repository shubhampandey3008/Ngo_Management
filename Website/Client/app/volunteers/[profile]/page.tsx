...
async function getVolunteer(profile: string | string[] | undefined): Promise<Volunteer> {
  const res = await fetch(`http://192.168.22.180:3000/volunteer/${profile}`)
  const data: Volunteer = await res.json()
  return data
}

async function fetchStudentData(studentId: string) {
  const resp = await fetch(`http://192.168.22.180:3000/student/${studentId}`)
  const studentProfile: Student = await resp.json()
  return { name: studentProfile.name, url: `http://192.168.22.180:3000/student/${studentId}` }
}
